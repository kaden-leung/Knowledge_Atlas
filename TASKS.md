...
| UJ-10 | **~~Expand VR-measurability page with IATs / Q-sort / adaptive preference + specific sensor inventory~~** | CW | **COMPLETED 2026-05-18.** Per DK 2026-05-18 review. Family 2 now has IAT (Greenwald 1998), Q-sort / 20-card sort (Stephenson 1953; Watts & Stenner 2012), adaptive-preference / discrete-choice (Green & Srinivasan 1990; Toubia 2007). Family 4 rewritten with confirmed/maybe/request hardware tiers: EDA, HR, HRV, respiration, 3D wrist accelerometer (confirmed); eye tracking, pupillometry (Quest Pro / Vive Pro Eye / Varjo only); motion-tracking add-ons, surface EMG (request). Quick-reference table now uses short codes (f1.head_pose, f2.iat, f4.eda, ...) for substitution-skill consumption. Commit `bb3c316`. |
| UJ-11 | **~~DYK CTAs + rename Pursue → Explore~~** | CW | **COMPLETED 2026-05-18.** Surface 2 DYK cards renamed and given three CTAs per VR-tractable card (Explore this topic / Add to short-list / Find related papers); contested cards get a fourth (Why is this contested? → opens Surface 7b). Non-VR-tractable cards get short-list-for-reference + Why-not-tractable links. Commit `1178cd8`. |
| UJ-12 | **~~Surface 3 short-list + logged-in + hot anchor-paper links~~** | CW | **COMPLETED 2026-05-18.** Anchor paper rows are now hot links to ka_article_view.html?id=PDF-XXXX (Track-4 owns target page). Pick column splits into short-list + pick. New short-list card above commit card shows persistent collection. Three new dependencies named (short-list persistence schema, hot-link URL contract, DYK short-list write path). Commit `1178cd8`. |
| UJ-13 | **~~Surface 4 two-path architecture: in-corpus vs uploaded (V7-Lite)~~** | CW | **COMPLETED 2026-05-18.** Path A (in-corpus) keeps the Ulrich 1984 example unchanged. Path B (uploaded) specifies the V7-Lite pipeline: paper-type classifier → embedding similarity → IV/DV extraction → VR-suitability mapping → conditional VOI → recommendation. Worked example (Lee/Park/Schultz 2025 biophilic-VR) shows admit-with-substitution; rejection example (meditation/default-mode paper, below similarity threshold) shows the third outcome. Six new dependencies named. Commit `1178cd8`. |
| UJ-14 | **~~Cook & Campbell four-validities showcase surface (Surface 7b)~~** | CW | **COMPLETED 2026-05-18.** Per DK 2026-05-18: "You must also showcase Cook & Campbell's classic four validities." Four-card grid per Shadish/Cook/Campbell 2002 §2: internal (8 threats), external (3 threats), construct (6 threats), statistical conclusion (6 threats), each with VR-project advisory. Worked example: why Meyers-Levy & Zhu 2007 ceiling-height finding is contested (construct + internal validity puzzle). Self-check page contemplated as follow-on. Commit `1178cd8`. |
| UJ-15 | **~~Sprint plan + farming-out recommendation~~** | CW | **COMPLETED 2026-05-18.** Answers DK's question "Can you do them all? Any reason to farm them out?" Eight-sprint plan with owners: A (CW content), B (CW + Track 4 HTML), C (Codex V7-Lite — farm out), D (Codex + AG substitution skill, panel-blocked), E (Codex + Track 4 short-list + viewer — farm out), F (CW + DK + 8 panelists VOI round-trip), G (Codex + AG citation-graph), H (CW methodological-pitfalls page, panel-blocked). Two-thirds of work is CW's lane; one-third should be farmed to Codex/AG with CW writing specs. `docs/UJ_SPRINT_PLAN_2026-05-18.md`. |
| UJ-A1 | **~~Cook & Campbell content lifted to standalone markdown~~** | CW | **COMPLETED 2026-05-18.** `160sp/ka_validities_showcase_content_2026-05-18.md`. Source-of-truth for Surface 7b; same pattern as the VR-measurability page. Includes worked example of ceiling-height contestation as construct + internal validity puzzle, self-check page contemplated as follow-on. References to Cohen 1988, Cook & Campbell 1979, Shadish-Cook-Campbell 2002 et al. Commit `[next]`. |
| UJ-A4 | **~~Substitution-skill build-spec document for Codex~~** | CW | **COMPLETED 2026-05-18.** `docs/SUBSTITUTION_SKILL_SPEC_2026-05-18.md`. Three-table substitution graph (constructs, measures, construct_measure_links). Corpus-wide extraction prompt (AG runs). Admit-mode and choice-mode API contracts. Refusal criteria from Mayo+Machery synthesis positions. Confidence display: actual numbers not bands per Pearl+Gelman+Bergstrom convergent position. Owner timeline ~3 weeks. Commit `[next]`. |
| UJ-A5 | **~~V7-Lite build-spec document for Codex~~** | CW | **COMPLETED 2026-05-18.** `docs/V7_LITE_SPEC_2026-05-18.md`. Seven synchronous stages (paper-type classification → embedding similarity → IV/DV → methods → VR-suitability mapping → conditional VOI [4-target subset] → recommendation synthesis); async full-V7 queue for the rest. Database writes to ae.db V2 schema with v7_lite_partial flag. Topic-similarity threshold calibration protocol per-topic. Owner timeline ~3 weeks. Commit `[next]`. |
| UJ-F-sim | **~~VOI panel: CW-simulated synthesis~~** | CW | **COMPLETED 2026-05-18.** `docs/VOI_PANEL_SYNTHESIS_2026-05-18.md`. Each of eight panelists has a ~700-900 word position based on their published work, clearly labelled as CW-simulation pending real panel. Convergent positions (V2 credence schema as substrate, profile not score, structured finder coupling, severity at the centre). Five contested positions for DK adjudication. Working operationalisation proposed. Unblocks UJ-A4, UJ-A5, UJ-D, UJ-H; does NOT close real-panel sprint UJ-F. Commit `b1fb190`. |
| UJ-A6 | **AG ae.db V2 credence schema as VOI substrate — design integration** | CW + AG | The panel synthesis (UJ-F-sim) and the substitution-skill + V7-Lite specs (UJ-A4, UJ-A5) all assume AG's V2 schema is the substrate. Action items: (1) review AG's `docs/CLAUDE_HANDOFF_V2_CREDENCE_2026-05-18.md` against panel positions for any missed substrate; (2) ensure ae.db's three-credence display, warrant-richness fields, severity, scope metadata, theory entrenchment, and weak-mechanism-link records are surfaced in the topic-page and article-viewer UI; (3) write a small reference doc bridging the Knowledge_Atlas-side renderers and the Article_Eater-side data model. Unblocked. |
