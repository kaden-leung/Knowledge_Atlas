...
**Finding:** Yes — SC3 steps 3, 4, 5, 6 all have `confidence: null` and `warrant: null` at the JSON level. The original threshold-based detection (`confidence < 0.5`) would skip them entirely. But these are the highest-VOI gaps in the corpus (Holl-vs-Ellard threshold debate). Across the full 554-gap corpus, 49 gaps have `confidence: null`.
**Fix:** Added "null confidence implies low confidence" rule in `extract_gaps()`. A null value is operationalized as **unresolved epistemic status with a provisional uncertainty prior of 0.65** (i.e., provisional confidence=0.35). "Null" here does not mean "zero confidence" — it means the panel declined to assign a numeric estimate, which is itself an epistemic signal: the mechanism is not yet characterized well enough to be assigned a credence. The 0.35 provisional value is a conservative prior, not an inferred measurement. Null-confidence gaps are identifiable in the output by `confidence: null` paired with `voi_components.uncertainty: 0.65` — a dedicated `confidence_imputed: true` surface field is planned for schema v3.4. `warrant: null` triggers `warrant_source: "absent"`.
**Why 0.35 and not lower:** The lowest explicitly assigned confidence observed in the corpus is **0.30** (verified by `min(g['confidence'] for g in gap_report.json['gaps'] if g['confidence'] is not None)`). Treating null as 0.35 places these gaps just above the corpus floor and below the structured-but-uncertain range (0.45–0.55). A value of 0.0 or 0.1 would falsely imply the panel assessed the claim as near-certainly false, not as uncharacterized.
