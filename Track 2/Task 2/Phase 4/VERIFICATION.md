...
**Fix:** Added "null confidence implies low confidence" rule in `extract_gaps()`. A null value is operationalized as **unresolved epistemic status with a provisional uncertainty prior of 0.6** (i.e., provisional confidence=0.4). "Null" here does not mean "zero confidence" — it means the panel declined to assign a numeric estimate, which is itself an epistemic signal: the mechanism is not yet characterized well enough to be assigned a credence. The 0.4 provisional value is a conservative prior, not an inferred measurement. The output field `confidence_imputed: true` marks these gaps for downstream consumers to treat accordingly. `warrant: null` triggers `warrant_source: "absent"`.
**Why 0.4 and not lower:** The lowest explicitly assigned confidence in the corpus is 0.35. Treating null as 0.4 keeps null-confidence gaps marginally below the structured-but-uncertain range (0.45–0.55) while placing them clearly above the definitional floor. A value of 0.0 or 0.1 would falsely imply the panel assessed the claim as near-certainly false, not as uncharacterized.
