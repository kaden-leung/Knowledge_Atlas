...
- Gap extraction lives in `gap_extractor.py` in the external **Article_Eater** repository (not bundled in this submission — it is the upstream system Task 2 consumes).
- Query generation lives in [Track 2/Task 2/Phase 3/query_generator.py](<Phase 3/query_generator.py>).
...
- The query generator is deterministic aside from `generated_at`; the determinism check is documented in [MANIFEST.md](MANIFEST.md).
