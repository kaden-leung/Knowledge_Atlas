...
**6 new gap-driven queries** were generated by the query generator and run through the Phase 2 search runner with SerpAPI and scholarly enabled. The run used **6 SerpAPI credits**, produced **120 raw results**, deduped to **67 candidates**, and recorded **0 null-result queries** in [Phase 2/search_results_new_templates.json](<Phase 2/search_results_new_templates.json>). The artifact boundary is documented in [Phase 2/EXPERIMENT_ONLY.md](<Phase 2/EXPERIMENT_ONLY.md>). No exact gold-standard DOI matches surfaced from automated retrieval — the results were adjacent literature in the correct subfields, not the specific canonical papers. To keep the evaluated pipeline DB at its cleanly-assessed **10-ACCEPT** state, these candidates were analyzed as a retrieval experiment and **not merged into the committed DB**; the recall gains reported below come from the Scholar Labs (AI-citation) runs.
...
### Note on the SerpAPI/scholarly variant (not merged into the evaluated DB)

Had the 67 experiment candidates been loaded, they would have added ~11 ACCEPT rows in adjacent subfields (birdsong restoration, active inference, attention networks, neuroaesthetics) — none of them gold-standard papers. To avoid inflating the ACCEPT set with un-assessed adjacent literature, they were **left out of the committed pipeline DB**, which remains at its precision-assessed **10-ACCEPT** state (the set traced in `PROVEIT_WORKS.md` and scored in `BENCHMARK_EVALUATION.md`). The DB intentionally has **0 rows** with `discovery_run_id = 'RUN-20260603-020713'`. The recall improvement to 40% comes entirely from the Scholar Labs runs above (Corbetta & Shulman #29, Kok #13), which are independent of the committed DB's ACCEPT count.
